   ...
   return (
    <div className="sm:col-span-3 col-span-4 grid grid-rows-[20px_1fr_20px] items-center justify-items-center p-3 pb-20 sm:p-10 font-[family-name:var(--font-geist-sans)] z-10 bg-white bg-opacity-90 dark:bg-opacity-90 dark:bg-slate-800">
      ...
      <section id="about">
         ...
         ユーザーのクローゼット管理サービス「WADRO(ワドロ)」(仮称、{WrapCode('以下WADRO')})のプロジェクト実施です。<br /><br />
         ...
         WADROによって、購入〜管理〜買取までを網羅し、全ての流れを{WrapCode('zozoサービス内で完結できるようにする')}ことが最終目標です。</p>
         ...
         <div className="p-5 border-l-2 pe-0 sm:pe-5">
            ...
            <div className="relative sm:rounded-lg mb-4 grid overflow-hidden w-fit">
               <p className="font-semibold">持ち服管理</p>
               <p className="leading-7 mb-3">ユーザー自身のクローゼットやチェストで保管している服やアクセサリーを登録できる機能です。<br />
               画像などの商品データを手動で登録することでデータの登録が可能です。<br />
               ZOZO TOWN内で販売している商品は連携により、データの取得ができます。</p>

               <hr className="my-5" />

               <p className="font-semibold">欲しいモノ管理</p>
               <p className="leading-7 mb-3">欲しい服やアクセサリーなどのアイテムが登録できる機能です。<br />
               ZOZO TOWNやZOZO USEDでお気に入り登録した商品は自動的にこちらのリストに登録されます。</p>

               <hr className="my-5" />

               <p className="font-semibold">スケジュールコーデ管理</p>
               <p className="leading-7 mb-3">カレンダーと連携して、持ち服管理に登録したアイテムで、その日毎のコーデをあらかじめ登録しておける機能です。<br />
               登録済みのコーデは別日へのコピー可能で、一般への公開も可能です。</p>

               <hr className="my-5" />

               <p className="font-semibold">ランドリー管理</p>
               <p className="leading-7 mb-3">洗濯中、クリーニング中の衣類を管理できる機能です。<br />
               持ち服管理とも連動し、洗濯中の衣類に関しては「洗濯中」などのバッジ付与。<br />
               また、コーデとしてはグループ化できないなどの制限をつけることができます。</p>
            </div>
         </div>
      ...
      <section id="diff">
         ...
         例えば、{WrapCode('持ち服管理')}の機能であれば{WrapCode('WEAR')}内の「お持ちのアイテム」が存在しますし、<br />
         {WrapCode('欲しいモノ管理')}であれば{WrapCode('WEAR、ZOZOTOWN')}のどちらにも「お気に入り」機能が存在しています。<br />
         こちらについて以下より、機能毎の詳細情報や、既存サービスとの相違点について記載します。</p>
         ...
         <div className="p-5 border-l-2 pe-0 sm:pe-5">
            ...
            <div className="relative sm:rounded-lg mb-4 grid overflow-hidden w-fit">
               ...
               そこで新たに「購入価格」というカラムを作成し、ユーザーが購入した時点の金額を登録できるように。<br />
               また、「買取金額」カラムを作成し連携先のフリマサイトより、現在の最高買取金額を取得して表示できるようにします。<br /><br />
               さらに、表示方法としてWEARアプリ内では「お持ちのアイテム」はフォルダ分け表示のみ対応していますが、<br />
               これをタブ表示できるようにします。<br />
               ...
               「{WrapCode('お気に入りは、内部購入につながる機能のため、内部のみ登録可とすることで外部への流出を最小限にしたい')}」という観点から、<br />
               この機能はメリットがないように思うかもしれませんが、<br />
               ...
               本機能にはメリットも多く、<br />
               欲しい服を登録する際には「お持ちのアイテム」にある機能と同じように、ZOZOサービス内から同一アイテムを紐付けることによって、<br />
               最終的にはZOZO対応ECへの誘導することも可能です。<br />
               さらには、外部URLの登録が多数存在するアイテムやブランドの傾向を把握することができるなど、メリットも数多く存在します。</p>

               <hr className="my-5" />

               <p className="leading-7 mb-3">それら以外の機能に関しては、新規の機能なので、上の項目で説明した成果が挙げられます。</p>
      ...
      <section id="point">
         ...
         現時点で確認できないポイントによっては、実現できない機能が出てくる可能性もあります。<br />
         ...
         <div className="p-5 border-l-2 pe-0 sm:pe-5">
            <div className="relative sm:rounded-lg mb-4 grid overflow-hidden w-fit">
               ...
               <ul className="list-disc ms-5 mb-0">
                  <li>データベース上に参考価格のカラムや、新たに服のコーデ、すなわち服の組み合わせのグループテーブルが追加できるか</li>
                  <li>APIを経由している場合は、APIへの影響、さらには他テーブルとプライマリーキー制約に影響はないか</li>
         ...
         <div className="p-5 border-l-2 pe-0 sm:pe-5">
            <div className="relative sm:rounded-lg mb-4 grid overflow-hidden w-fit">
               ...
               <ul className="list-disc ms-5 mb-0">
                  ...
                  <li>ZOZO関連ECのデータベースに登録されていないアイテムの、買取金額のデータが存在するか</li>
               </ul>
            </div>
         </div>
